...
This repository contains the VHDL code for a multiplier using the Booth radix-4 algorithm, taking as inputs two signed numbers in 2's complement notation (N-bit multiplicand × 8-bit or 16-bit multiplier) and returning as output a signed (N + 8 or 16)-bit number in 2's complement notation. The Booth radix-4 algorithm computes the two inputs into M/2 partial products. These partial products are passed through an Wallace tree adder structure, which returns two final partial products. These two partial products are then subjected to a further carry-free addition (CLA adder) to produce the final result.  

The structural block diagram is as follows:
1. **Booth Encoder and Decoder:** Generation of partial products.
